...
const AdminAddProducts = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/category/view-cat"
      );
  ...
  const fetchSubCategories = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/category/view-subcat"
      );
      if (response.data.status) {
  ...
  let filteredProducts = subCategoryData.filter(
    ...
    <div>
      ...
      <Container className="d-flex flex-column justify-content-center">
        ...
        <Form
          ...
          className="card p-3 border-primary mt-4 gap-3"
        >
          ...
          <Form.Group controlId="subcategory">
            ...
            >
              {filteredProducts.map((category) => (
                <option key={category._id}>{category.subcategory}</option>
              ))}
