...
const AdminHandleuserPage = () => {
  ...
  const handleUpdate = async () => {
    ...
    if (checkName === null && checkEmail === null) {
      ...
      toast.error(
        "The username must be longer than 3 characters and must be a valid Gmail address"
      );
    }
  };
  ...
  const onRemoveUser = async (UserId) => {
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/auth/remove/${UserId}`
      );
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() =>
                          handleShow(user.username, user.email, user._id)
                        }
                      >
                        edit
                      </button>
